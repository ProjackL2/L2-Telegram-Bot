...
- Установленная Java 23 или новее в системном PATH (`java -version`).
...
- *Примечание*: При первом запуске Hub потребуется лицензия. Подробности см. в разделе [Лицензирование](licensing.md).
...
### Дополнительные настройки игрового сервера
Скрипт интеграции должен присутствовать на игровом сервере и должен называться `AgentScript.java`.

По умолчанию скрипт интеграции содержит обработчик bypass для привязки персонажа через QR-код.

Пример кнопки bypass:
```
<button value="tg" action="bypass -h linktg" width=90 height=24 back="L2UI_CT1.Button_DF_Down" fore="L2UI_CT1.Button_DF">
```

Не забудьте добавить `linktg` в конфигурацию `InitialBypassEncodeIgnore` в `security.ini` игрового сервера.
