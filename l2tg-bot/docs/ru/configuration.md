...
Открытие портов критически важно, так как Hub и Agent взаимодействуют по протоколу RMI (Remote Method Invocation). Для корректной работы системы Hub должен принимать входящие соединения от агентов (по умолчанию `1099`), а каждый Agent должен принимать обратные вызовы от Hub (по умолчанию `1100`). Если эти порты заблокированы брандмауэром, компоненты не смогут обнаружить друг друга или обмениваться данными, что приведет к ошибкам соединения.

[Как открыть порт в Windows Server](https://www.kamatera.com/knowledgebase/how-to-open-a-port-in-windows-firewall/)

[Как открыть порт в Linux](https://www.digitalocean.com/community/tutorials/opening-a-port-on-linux)
